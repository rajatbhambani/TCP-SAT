...
\begin{itemize}
  \item The BBR-SAT algorithm: two new signals, an orbit parameter table,
        ...
        $<$100 lines of new state and signal-handling code (\S\ref{sec:design}).
  \item A zero-loss, shared-link QUIC simulator (picoquic-based) covering
        ...
        six lead times (\S\ref{sec:methodology}).
...
\texttt{bw\_hi} (long-term delivery-rate ceiling, updated on loss events),
...
A separate two-slot windowed maximum filter (\texttt{MaxBwFilter}) tracks
the short-term peak delivery rate over the last two ProbeBW cycles.
The algorithm cycles through four ProbeBW sub-states ---
DOWN, CRUISE, REFILL, UP --- to probe for spare capacity while maintaining
a near-full pipe.

The critical weakness on abrupt downward transitions is the
\texttt{MaxBwFilter}: a two-slot windowed max spanning two complete ProbeBW
cycles, each lasting $\sim$4\,$\mathrm{RTT}_\mathrm{new}$, means the pacing
rate remains calibrated to the old orbit for approximately
...
The simulator's queuing is soft-limited by a $2\times$RTT delay threshold;
the \emph{effective} buffer in our experiments is uncapped by that threshold
(packets continue to queue if the sender does not back off), which is why
Table~\ref{tab:exp1_leo_geo} shows a 33\,MB build-up for B1/B3 rather
than the $\sim$725\,KB hard-BDP limit.
...
a production deployment would carry them in a QUIC DATAGRAM frame or
operator-defined transport parameter.
