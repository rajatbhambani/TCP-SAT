...
Peak Q reflects on-path data; higher values indicate better buffer utilisation,
not bloat, when the algorithm converges.}
\end{table}
...
$^\S$ B4 catastrophic: retransmit storm on zero-duration pause; 45\,MB queue, $<$1\,MB goodput.}
\end{table}
...
measurable goodput cost; a production deployment could gate the full switch
on the source-to-target BW ratio to suppress it for transitions where
