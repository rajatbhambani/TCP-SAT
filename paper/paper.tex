...
in 4.5\,s for the critical LEO$\to$GEO transition under a realistic
1$\times$BDP droptail buffer cap --- the only zero-loss algorithm to converge;
...
($T_\mathrm{HO}=30$\,s, no loss, uncapped queue shown for reference).
Under the 1$\times$BDP cap, BBR-SAT achieves T90\,=\,4.5\,s at $\ell=0$
and 1.5\,s at $\ell=2$\,s; B1/B3/B4 are N/C at all lead times.
...
LEO$\to$GEO transition (uncapped queue, shown for reference).
Under the primary 1$\times$BDP cap, BBR-SAT achieves T90\,=\,4.5\,s
at $\ell = 0$; a 2-second lead time reduces this to 1.5\,s.
...
After the handover, the RTT triples from 50\,ms to 150\,ms; BBR's
...
(62\,KB).
At the higher RTT, this window yields only $62\,\text{KB}/150\,\text{ms}
\approx 3.3$\,Mbps --- well below the 9\,Mbps threshold --- for the
