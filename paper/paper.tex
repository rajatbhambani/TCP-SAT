...
\textbf{Large-buffer reference} (uncapped queue):
Without a droptail limit, B1 and B3 exhibit a brief transient
(T90\,=\,12.5\,s), throughput briefly crossing 2.7\,Mbps for a single
one-second window before collapsing to near-zero (2.3\,MB goodput, 12\,MB
peak queue).
BBR-SAT converges in 2.5\,s with a $44\times$ lower peak queue (286\,KB
vs.\ B1/B3's 12\,544\,KB) and 18.6\,MB goodput; CUBIC converges in 10.5\,s.
These numbers characterise unmanaged satellite ground-station buffers;
the 1$\times$BDP cap above is the primary evaluation model.
...
Under the 1$\times$BDP cap, B4 fails catastrophically for the
GEO$\to$LEO transition (N/C, $<$1\,MB goodput, 45\,MB peak queue despite
the cap).
With $\ell = 0$, PREDICTED and CONFIRMED fire simultaneously;
B4's zero-duration pause triggers an immediate retransmission storm on
resume that overwhelms the LEO link.
This points to a fundamental fragility of send-pause/resume: with no
advance notice the mechanism collapses rather than gracefully degrading.
...
With the corrected asymmetric link model, LEO and MEO share the same
upload bottleneck rate (10\,Mbps), so the T90 threshold (9\,Mbps,
i.e.\ 90\,\% of 10\,Mbps) requires sustaining near-maximum throughput
immediately after the handover.
After the handover, the RTT triples from 50\,ms to 160\,ms; BBR's
\texttt{min\_rtt} filter retains the stale 50\,ms estimate for up to
10\,s, capping the effective inflight window at BDP$_\mathrm{LEO}$
(62\,KB).
At the higher RTT, this window yields only $62\,\text{KB}/160\,\text{ms}
\approx 3.1$\,Mbps --- well below the 9\,Mbps threshold --- for the
duration of the filter window, explaining the N/C result for all BBR
variants.
CUBIC's loss-driven window growth from an undersized initial cwnd
overcomes the RTT transition in 5.5\,s.
This N/C is a \emph{metric artifact}: the actual bottleneck capacity
is unchanged at 10\,Mbps throughout, and all algorithms sustain near-full
utilisation in steady state both before and after the handover.
Resolving the MaxBwFilter plateau would require artificially inflating
bandwidth estimates beyond observed delivery rates --- a modification that
would compromise BBRv3's model-based design and is outside the scope of
this extension.
...
Under the 1$\times$BDP cap, BBR-SAT achieves T90\,=\,1.5\,s for all
tested lead times at $T_\mathrm{HO} = 60$\,s, an improvement over the
4.5\,s baseline at $T_\mathrm{HO} = 30$\,s.
The longer pre-handover window allows more of the LEO orbit table entry's
\texttt{bw\_hi} probe cycles to accumulate before CONFIRMED fires,
reducing the time needed in REFILL.
At $T_\mathrm{HO} = 120$\,s, convergence degrades: T90\,=\,4.5\,s at
$\ell = 0$, improving to 1.5\,s at $\ell = 2$\,s.
The orbit table's 10-second \texttt{min\_rtt} expiry allows the seeded
GEO RTT entry to become partially stale before the handover fires at
120\,s; a 2-second lead time re-seeds \texttt{min\_rtt} just before
...
Both flows start on the 3\,Mbps GEO beam from $t = 0$.
...
BBR-SAT 1.31\,Mbps, CUBIC 1.58\,Mbps, $J = 0.887$.
The 55/45 CUBIC/BBR-SAT split is consistent across both runs
(run-to-run $\sigma < 0.1$\,Mbps).
