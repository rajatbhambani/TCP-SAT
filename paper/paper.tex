...
while reducing peak queue occupancy by $44\times$ relative to vanilla BBRv3.
...
accumulating $\approx$12\,MB of queued data and delivering only 2.3\,MB
total goodput in a 90-second window --- $8\times$ below BBR-SAT's 18.6\,MB.
...
builds an arbitrarily large queue; the 12\,MB peak for B1 is this failure.
...
\textbf{B1 (vanilla BBRv3)} records T90 = 12.5\,s, but this is a transient
rather than stable convergence: the delivered throughput briefly crosses
90\% of the 3\,Mbps GEO capacity (2.7\,Mbps) for a single one-second window
at $t + 12.5$\,s before collapsing to near-zero, yielding only 2.3\,MB
total goodput over the 90-second run.
The on-path queue inflates to 12\,MB (peak) as BBRv3 retains its
LEO-calibrated 10\,Mbps bandwidth estimate after handover.
...
the sender continues pacing at 10\,Mbps into a 3\,Mbps pipe,
building a queue that prevents sustained delivery.
The BBRv3 pacing-rate architecture: because pacing rate is set from the
\texttt{MaxBwFilter} (not the congestion window), the queue cannot be
drained by cwnd manipulations alone.

\textbf{B3 (cwnd-freeze)} exhibits the same failure mode with identical
numbers (T90 = 12.5\,s transient, 2.3\,MB goodput, 12\,MB peak queue).
Freezing the congestion window at the pre-handover LEO value leaves an
equally oversized inflight in place; the send rate is governed by the
pacing rate (still 10\,Mbps), not the cwnd, so the freeze has no effect.
Advance warning is useless: even at $\ell = 30$\,s, B3 does not converge
to a stable throughput above the 90\% threshold.
...
loads the seeded GEO orbit entry (\texttt{bw\_hi} = 3\,Mbps, \texttt{min\_rtt}
= 580\,ms), enters a brief DRAIN phase to clear residual in-flight data,
then transitions to REFILL and converges to a steady-state of 2.9\,Mbps
(97\,\% of link capacity) with a peak queue of only 286\,KB --- a
$44\times$ reduction compared to B1/B3.
Goodput over the 90-second run is 18.6\,MB, largely independent of lead time.
...
\textbf{CUBIC} converges in 10.5\,s at all lead times, achieving a
steady-state of 3.1\,Mbps ($\approx$100\,\% utilisation) and 19.2\,MB goodput.
The convergence is loss-driven: the queue overflow immediately after
handover triggers multiplicative window reduction, and CUBIC settles within
roughly 18 GEO RTTs ($\approx$10.5\,s).
Although CUBIC achieves slightly higher single-flow goodput than
BBR-SAT (at the cost of a 307\,KB peak queue versus 286\,KB), its
...
competitive; BBR-SAT's advantage lies in zero-loss operation and
mixed-CCA deployments where BBRv3 flows must coexist fairly on a shared
beam --- the common operational scenario for multi-orbit gateway links.

\textbf{Buffer sensitivity} ($N=10$, 1$\times$BDP droptail cap of 218\,KB):
Under the constrained buffer, B1 never converges (steady-state utilisation
69\,\%, 13.7\,MB goodput) because BBRv3's pacing-rate architecture
cannot drain the queue via cwnd manipulation alone --- the same root cause
as the uncapped case, but now the queue ceiling prevents even a transient
T90 trigger.
BBR-SAT converges loss-free in 4.5\,s at 99\,\% utilisation (20.1\,MB
goodput); CUBIC converges via loss in 5.5\,s at 97\,\% (19.6\,MB).
BBR-SAT delivers 46\,\% more goodput than B1 under the cap.
...
\midrule
B1 BBRv3 (vanilla) & 12.5\,s$^\dagger$ & 0.0\,Mbps & 2.3\,MB  & 12\,544\,KB \\
B3 cwnd-freeze     & 12.5\,s$^\dagger$ & 0.0\,Mbps & 2.3\,MB  & 12\,544\,KB \\
B4 pause/resume    & N/C               & 0.0\,Mbps & 1.9\,MB  & 12\,329\,KB \\
BBR-SAT (ours)     & \textbf{2.5\,s} & \textbf{2.9\,Mbps} & \textbf{18.6\,MB} & \textbf{286\,KB} \\
CUBIC              & 10.5\,s & 3.1\,Mbps & 19.2\,MB & 307\,KB \\
\bottomrule
\end{tabular}
\vspace{2pt}

{\footnotesize $^\dagger$ Transient only: throughput crosses 2.7\,Mbps
($90\%$ of 3\,Mbps) for one window at $t+12.5$\,s then collapses;
goodput is $8\times$ below BBR-SAT.}
\end{table}
...
LEO$\to$GEO  & 12.5\,s$^\dagger$ & 12.5\,s$^\dagger$ & N/C & \textbf{2.5\,s} & 10.5\,s \\
MEO$\to$LEO  & 0.5\,s & 0.5\,s & 0.5\,s & 0.5\,s  & 0.5\,s \\
GEO$\to$LEO  & 1.5\,s & 1.5\,s & 0.5\,s & 1.5\,s  & 0.5\,s \\
MEO$\to$GEO  & 1.5\,s & 1.5\,s & 1.5\,s & 1.5\,s  & 1.5\,s \\
GEO$\to$MEO  & 2.5\,s & 2.5\,s & 2.5\,s & 2.5\,s  & 0.5\,s \\
\bottomrule
\end{tabular}
\vspace{2pt}

{\footnotesize $^\dagger$ Transient only; see Table~\ref{tab:exp1_leo_geo} and \S\ref{sec:exp1}.\\
...
(0--20\,s), achieving T90 = 2.5\,s with a $44\times$ reduction in peak
queue occupancy relative to vanilla BBRv3 and $8\times$ more goodput
(18.6\,MB vs.\ 2.3\,MB over a 90-second run).
