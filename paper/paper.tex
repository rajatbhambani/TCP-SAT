...
(up to $3.3\times$ upload reduction) and round-trip time (up to $12\times$
increase), which existing congestion-control algorithms handle poorly:
...
a LEO$\to$GEO handover simultaneously reduces the upload bottleneck rate by
$3.3\times$ (10$\to$3\,Mbps) and increases the round-trip propagation delay
by $\sim$$12\times$ (50$\to$580\,ms).
Because the GEO BDP (3\,Mbps$\times$580\,ms\,=\,218\,KB) exceeds the
LEO BDP (10\,Mbps$\times$50\,ms\,=\,62\,KB) by $3.5\times$, the \emph{new}
orbit requires a \emph{larger} inflight window --- but at a \emph{much lower}
pacing rate.
